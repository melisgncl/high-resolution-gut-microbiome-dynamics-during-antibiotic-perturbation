...
from _common import (COLONISED, diversity, jacobian, match_nearest, stats,
                     style, to_days, WINDOW, WINDOW_PRIMARY)
...
def main():
    ...
    ax.axvline(WINDOW_PRIMARY, color="#C0392B", ls="-", lw=1.2, zorder=0,
               label=f"primary window ({WINDOW_PRIMARY})")
    ax.axvline(WINDOW, color="#7F7F7F", ls="--", lw=0.9, zorder=0,
               label=f"Figure 4's window ({WINDOW}) — control clock forces this")
    for w in sweep["window"]:
        ...
        ax.annotate(f"{nsig}/8", (w, sweep.loc[sweep["window"] == w, "rho"].iloc[0]),
                    ...
                    bbox=dict(fc="white", ec="none", pad=0.5, alpha=.85))
    ...
    ax2.set_title(f"B   All mice pooled, window = {WINDOW_PRIMARY}")
    fig.tight_layout()
